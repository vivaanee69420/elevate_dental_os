...
import {
  ...
  useGoogleCampaigns, useGoogleAdGroups, useGoogleAds, useGoogleKeywords, useGoogleSearchTerms,
...
export function GoogleSummary() {
  ...
  const real = <T extends { id: string | null }>(rows: T[]) => rows.filter((r) => r.id !== null);

  const bestCampaign = bestByCostPerConversion(real(campaigns.data?.rows ?? []));
  ...
  const grains: Grain[] = [
    {
      label: 'Best campaign · cost per conversion', row: bestCampaign, fallbackName: 'Unnamed campaign',
      note: noteFor(campaigns.data?.rows, bestCampaign),
      href: '/marketing-google?tab=campaigns',
    },
    {
      label: 'Best ad group · cost per conversion', row: bestAdGroup, fallbackName: 'Unnamed ad group',
      note: noteFor(adGroups.data?.rows, bestAdGroup),
      href: '/marketing-google?tab=adgroups',
    },
    {
      label: 'Best ad · cost per conversion', row: bestAd, fallbackName: 'Unnamed ad',
      note: noteFor(adRows, bestAd),
      href: '/marketing-google?tab=ads',
    },
    {
      label: 'Best keyword · cost per conversion', row: bestKeyword, fallbackName: 'Unnamed keyword',
      note: noteFor(keywordRows, bestKeyword),
      href: '/marketing-google?tab=keywords',
    },
    {
      label: 'Best search term · cost per conversion', row: bestTerm, fallbackName: 'Unnamed search term',
