...
vi.mock('../src/repositories/cockpit.repository.js', () => ({
  cockpitRepository: {
    ...
    leadsDetailRows: vi.fn(async () => [
      { id: 'l1', ghl_pipeline_id: 'fb1', practice_id: 'P1', created_at: '2026-07-02T10:00:00Z', contacts: { first_name: 'Jane', last_name: 'Doe', phone: '07700 900111', email: 'jane@doe.com' } },
    ...
    cashupDaysDetailRows: vi.fn(async () => [
      { cashup_date: '2026-07-02', practice_id: 'P1', business_name: 'Ashford', cash_up_money_taken_pence: 185000, detail_patient_money_total_pence: 175000, refunds: [{ amountPence: 5000, reason: 'goodwill' }], practices: { name: 'Ashford' } },
      { cashup_date: '2026-07-01', practice_id: null, business_name: 'Unmapped Biz', cash_up_money_taken_pence: 50000, detail_patient_money_total_pence: 60000, refunds: [], practices: null },
    ]),
  },
}));
...
describe('cockpitService.leadsDetail', () => {
  ...
  it('threads practiceId + limit/offset to the repo reads', async () => {
    ...
    expect(cockpitRepository.pipelineChannelMap).toHaveBeenCalledWith('org1', 'P1');
