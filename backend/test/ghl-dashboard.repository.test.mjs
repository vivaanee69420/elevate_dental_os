...
describe('aggregate', () => {
  it('calls the RPC with org/window/practice/account args and returns rows', async () => {
    ...
    expect(supaRec.rpcCalls[0]).toEqual({
      fn: 'ghl_dashboard_aggregate',
      params: { p_org: 'org-1', p_since: '2026-01-01T00:00:00Z', p_until: '2026-02-01T00:00:00Z', p_practice: null, p_account_id: null },
    });
  });

  // RPC v3 groups by integration_account_id, so a single subaccount is scoped
  // via p_account_id — NOT via its practice (two accounts can share a practice,
  // and unmapped accounts have none).
  it('forwards an explicit accountId as p_account_id', async () => {
    supaRec.rpcProvider = () => ({ data: [], error: null });
    await ghlDashboardRepository.aggregate('org-1', 's', 'u', null, 'acc-7');
    expect(supaRec.rpcCalls[0].params.p_account_id).toBe('acc-7');
    expect(supaRec.rpcCalls[0].params.p_practice).toBeNull();
  });
