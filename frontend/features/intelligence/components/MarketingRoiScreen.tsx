...
// View-level ad-account filter. Facebook (Meta) and Google get their own
// labelled rows, each selecting independently — pick one of each to compare a
// Facebook account against a Google account side by side, or one alone to see it
// separately. "All accounts" on a row clears just that provider. Each row hides
// when its provider has nothing to filter; the whole block hides when there's
// fewer than two accounts total. Owner-only endpoints — non-owners just don't get
// the rows (queries return no data → both render null).
function AdAccountFilter({
  metaId,
  googleId,
  onSelectMeta,
  onSelectGoogle,
