...
### `GET /api/wealth/inputs` — the saved personal balance sheet + assumptions, or sane empties when never configured. Returns `{ assets:[{name,valuePence,type,growthPct,liquid}], liabilities:[{name,valuePence,ratePct}], pensions:[{name,balancePence,contributionsYtdPence,type}], properties:[{name,address,valuePence,mortgagePence,monthlyIncomePence,monthlyCostPence,yieldPct,type}], fire:{targetAnnualSpendPence,withdrawalRatePct,growthRatePct,annualSavingsPence,horizonYears}, sale:{ownerSharePct,businessDebtPence,freeholdEquityPence,acquisitionCostPence,badrLifetimeUsedPence,useLiveValuation,enterpriseValuePence}, updatedAt }`. wealth.view.
...
### `GET /api/wealth/fire` — the assembled live Exit Plan. Resolves business EV from the **live valuation midpoint** (`sale.useLiveValuation`, else the manual `enterpriseValuePence`), runs the sale waterfall for net cash, then the FIRE projection. Returns `{ valuation:{enterpriseValuePence,source:'live'|'manual'}, waterfall:{equityValuePence,ownerEquityProceedsPence,ownerGainPence,badrGainPence,standardGainPence,cgtPence,effectiveCgtPct,netBusinessProceedsPence,freeholdEquityPence,totalNetProceedsPence}, fire:{currentNetWorthPence,fireNumberPence,gapPence,progressPct,sustainableAnnualIncomePence,targetAnnualSpendPence,withdrawalRatePct,growthRatePct,annualSavingsPence,horizonYears,yearsToFire,requiredAnnualSavingsPence,years:[{year,nwPence,hitFire}]}, inputs:{liquidAssetsPence,liabilitiesPence,fire,sale} }`. Business-type assets are excluded from the liquid pool (their realisable value comes from the waterfall, not double-counted). FORMULAS.md §Exit Plan. wealth.view.
### `POST /api/wealth/compute/sale-waterfall` — pure recompute of `formulas.calculateSaleWaterfall` (`saleWaterfallSchema`: `enterpriseValuePence,businessDebtPence,ownerSharePct,acquisitionCostPence,freeholdEquityPence,badrLifetimeUsedPence`). UK CGT: BADR 18% to the £1m lifetime cap, 24% above. **Audit-exempt.** wealth.view.
### `POST /api/wealth/compute/fire` — pure recompute of `formulas.calculateFirePlan` (`firePlanSchema`: `liquidAssetsPence,liabilitiesPence,businessNetProceedsPence,targetAnnualSpendPence,withdrawalRatePct,growthRatePct,annualSavingsPence,horizonYears`). **Audit-exempt.** wealth.view.
