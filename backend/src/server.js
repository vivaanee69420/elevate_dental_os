...
import { resumeInterruptedBootstraps } from "./lib/integrations/dentally-sync.js";
...
function start() {
    try {
        ...
        const server = app.listen(PORT, HOST, () => {
            ...
            // A deploy restarts this process, and the Dentally first pull is an
            // in-process job — so shipping while one runs kills it silently and
            // leaves a half-filled tenant. Pick those back up. Delayed so the
            ...
            setTimeout(() => {
                resumeInterruptedBootstraps()
                    .then((r) => { if (r?.resumed) console.log(`✓ resumed ${r.resumed} interrupted Dentally pull(s)`); })
