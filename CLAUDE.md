...
Local DB: the live Supabase project is rooted at repo-root `supabase/` (`config.toml` + `supabase/migrations/`). Run `supabase start` then `supabase db reset` **from the repo root** — it applies all `supabase/migrations/20260101*_*.sql` in numeric order. The current ledger runs `000001`→`000033`: schema, RLS, seed, `000004_access_token_hook` (creates the Custom Access Token Hook — critical, see rule 8), then RBAC/`role_permissions`, user status, business-health cadence, integrations (GHL/Dentally/Xero/CSV), `platform_admins`, the `auth_bootstrap` + rollup/payment-summary/settled-receipts RPCs, signup-approval, then the post-`000020` Dentally/analytics layer — `pms_patient_id`/`pms_practitioner_id` on appointments, growth practice-performance + chair-utilisation + treatment-mix/production RPCs, associate/staff Dentally mapping, `dentally_treatment_plans`, health manual metrics, GHL upsert indexes, and `lead_ghl_stage`. All are idempotent and re-apply cleanly. The `db/01_schema.sql`/`02_rls.sql`/`03_seed.sql` files are unmanaged source copies, **not** what `supabase db reset` reads; keep them in sync with `supabase/migrations/` when changing schema. After any hosted DDL run `NOTIFY pgrst, 'reload schema';` (PostgREST cache goes stale — recurring gotcha).
